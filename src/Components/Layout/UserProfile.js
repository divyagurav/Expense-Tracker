...
const UserProfile = (props) => {
  ...
  useEffect(() => {
    axios.post(
        'https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyD61trb_TQ27ZLrT4ybyyFKWkht-DaUa0o',
        {idToken: idToken}
    ).then((res) => {
        console.log(res);
    ...
    const updateProfiletHandler = async(event) => {
        ...
        try {
            const res = await axios.post (
                'https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyD61trb_TQ27ZLrT4ybyyFKWkht-DaUa0o'
                , updatedInfo 
            );
                console.log(res);
            } catch (err) {
